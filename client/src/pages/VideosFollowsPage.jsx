...
function VideosFollowsPage() {
  const { getVideos, deleteVideo, likeVideo, getLikes, getMyVideos,videos } = useVideos();
  ...
  useEffect(() => {
    // Obtener los IDs de los creadores que sigues utilizando la función getFollowedCreatorsRequest(id)
    const fetchFollowedCreators = async () => {
      try {
        const creatorIds = await getFollowedCreatorsRequest(user.id);
        console.log('creators id:', creatorIds.data);
        setFollowedCreators(creatorIds.data);
    ...
    <div className="container mx-auto flex flex-col items-center justify-center">
      <div className="flex justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-1 w-3/4 mt-0 ml-20 md:grid-cols-2 mt-2 lg:grid-cols-4 gap-2 p-2 mt-2">
          {videos
          .filter(video => followedCreators.includes(video.creador_id))
          .map((video, index) => (
            ...
            video.video_publicado && (
              ...
              >
                <p className="text-sm bg-slate-950 p-3">
...
export default VideosFollowsPage;
